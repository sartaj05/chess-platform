...
def serialize_tournament(row, user):
    return {
        ...
        "standings": [
            {"name": e.user.display_name, "score": str(e.score)}
...
class TournamentAPIView(views.APIView):
    ...
    def get(self, request, pk=None):
        if pk:
            return response.Response(serialize_tournament(Tournament.objects.get(pk=pk, is_public=True), request.user))
    ...
    def post(self, request, pk=None):
        ...
        try:
            join_tournament(tournament=row, user=request.user) if request.data.get(
                "action"
            ) == "join" else withdraw_from_tournament(tournament=row, user=request.user)

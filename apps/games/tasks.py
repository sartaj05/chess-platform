    ...
    for game in Game.objects.filter(status=Game.Status.ACTIVE).iterator():
        for color in ("white","black"):
            ...
            if disconnected and now >= disconnected + timedelta(seconds=game.reconnect_grace_seconds):
                ...
                apply_elo_ratings(game);count+=1;break
    return count
...
def evaluate_fair_play(game_id: str) -> dict:
    ...
    game=Game.objects.get(pk=game_id);reviews=MoveReview.objects.filter(game=game).select_related("move")
    ...
    if not reviews.exists() and game.ply_count:
        ...
        run_game_review(job=create_analysis_job(game=game, analysis_type="quick", depth=10))
        reviews=MoveReview.objects.filter(game=game).select_related("move")
    metrics={"white":[],"black":[]};matches={"white":0,"black":0}
    for review in reviews:
        color=review.move.color;metrics[color].append(review.score_loss_cp)
        if review.move_uci==review.bestmove_uci: matches[color]+=1
    ...
    signals=[];risk=0
    for color in ("white","black"):
        if len(metrics[color])>=12 and rates[color]>=80: signals.append(f"{color}_high_engine_match");risk+=35
        if len(metrics[color])>=12 and averages[color]<=18: signals.append(f"{color}_very_low_loss");risk+=30

...
class GameMoveInline(admin.TabularInline):
    model = GameMove
    extra = 0
    readonly_fields = ("ply_number", "move_number", "color", "uci", "san", "white_time_ms", "black_time_ms", "created_at")
    can_delete = False
...
class GameAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Players", {"fields": ("room", "white_user", "black_user", "white_display_name", "black_display_name", "rated", "allow_spectators")}),
        ("State", {"fields": ("status", "result", "termination", "winner_color", "turn", "ply_count", "fullmove_number", "last_move_uci", "last_move_san")}),
        ("Board", {"fields": ("initial_fen", "current_fen", "cached_pgn")}),
        ("Clock", {"fields": ("clock_initial_ms", "increment_ms", "delay_ms", "white_time_ms", "black_time_ms", "clock_started_at", "last_move_at")}),

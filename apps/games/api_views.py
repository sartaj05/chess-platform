...
class FairPlayReviewAPIView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        return Response([
            {"id": row.pk, "game": str(row.game_id),
             "players": f"{row.game.white_display_name} vs {row.game.black_display_name}",
             "status": row.status, "risk_score": row.risk_score,
             "engine_match": {"white": float(row.white_engine_match_rate),
                              "black": float(row.black_engine_match_rate)},
             "average_loss": {"white": row.white_avg_loss_cp, "black": row.black_avg_loss_cp},
             "signals": row.signals, "notes": row.moderator_notes}
            for row in rows[:100]
        ])

    def post(self, request, pk=None):
        review = FairPlayReview.objects.get(pk=pk)
        status_value = request.data.get("status")
        allowed = {FairPlayReview.Status.REVIEWING, FairPlayReview.Status.CONFIRMED,
                   FairPlayReview.Status.DISMISSED}
        if status_value not in allowed:
            return Response({"detail": "Invalid review decision."}, status=400)
        review.status = status_value
        review.moderator_notes = str(request.data.get("notes", ""))[:4000]

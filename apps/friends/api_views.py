...
class SocialAPIView(views.APIView):
    ...
    def get(self, request):
        friendships = Friendship.objects.filter(Q(requester=request.user)|Q(addressee=request.user)).select_related("requester","addressee")
        rows=[]
    ...
    def post(self, request):
        ...
        try:
            ...
            if action in {"accept","decline"}:
                item=Friendship.objects.get(pk=request.data.get("friendship_id")); respond_to_request(friendship=item,actor=request.user,accept=action=="accept"); return response.Response({"status":item.status})
            other=User.objects.get(pk=request.data.get("user_id"))
            if action=="message":
                chat=get_or_create_conversation(actor=request.user,other=other); message=send_message(conversation=chat,sender=request.user,body=request.data.get("body","")); return response.Response({"id":message.pk},status=201)
            if action=="challenge":
                room=create_room(request=request,cleaned_data={"name":f"{request.user.display_name} vs {other.display_name}","host_display_name":request.user.display_name,"mode":Room.Mode.ONLINE,"visibility":Room.Visibility.INVITE_ONLY,"clock_initial_minutes":int(request.data.get("minutes",10)),"increment_seconds":int(request.data.get("increment",0)),"delay_seconds":0,"color_preference":Room.ColorPreference.RANDOM,"rated":False,"allow_guests":False,"spectator_enabled":True})
                notify(recipient=other,kind=Notification.Kind.SYSTEM,title=f"Challenge from {request.user.display_name}",message=room.time_control_label,target_url=room.get_absolute_url()); return response.Response({"room_code":room.code,"invite_url":room.get_absolute_url()},status=201)
            if action=="block":

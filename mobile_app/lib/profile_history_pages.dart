        ...
        body: _busy && _profile == null
            ...
            : ListView(padding: const EdgeInsets.all(20), children: [
                CircleAvatar(
                    radius: 42,
                    child: Text(
                        (_displayName.text.isEmpty ? 'P' : _displayName.text[0])
                            .toUpperCase(),
                        style: const TextStyle(fontSize: 30))),
                const SizedBox(height: 12),
                Text(_profile?['email']?.toString() ?? '',
                    textAlign: TextAlign.center),
                Text('Bot level ${_profile?['bot_level'] ?? 1}',
                    textAlign: TextAlign.center,
                    style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 20),
                TextField(
                  ...
                  : _games.isEmpty
                      ? ListView(children: const [
                          Padding(
                              ...
                              child: Text('No saved online games yet.',
                                  textAlign: TextAlign.center))
                        ])
                      : ListView.separated(
                          ...
                          itemBuilder: (_, index) {
                            ...
                            return Card(
                              ...
                              leading: CircleAvatar(
                                  child:
                                      Text((game['result'] ?? '*').toString())),
                              title: Text(
                                  '${game['white_display_name'] ?? 'White'} vs ${game['black_display_name'] ?? 'Black'}'),
                              subtitle: Text(
                                  '${game['status'] ?? ''} · ${game['termination'] ?? 'in progress'}\n${game['created_at'] ?? ''}'),
                              isThreeLine: true,
